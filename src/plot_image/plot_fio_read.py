...
select_cpu_number_category = "select distinct cpu_number from fio_read where server_type=%(server_type)s"

select_frequency_category = "select distinct cpu_frequency from fio_read where server_type=%(server_type)s and cpu_number=%(cpu_number)s "

select_record = "select * from fio_read where server_type=%(server_type)s and cpu_number=%(cpu_number)s and cpu_frequency=%(cpu_frequency)s and memory_count=%(memory_count)s"

select_server_type_category = "select distinct server_type from fio_read "
...
for cpu_count_index in range(0, len(first_image_vcpu_count)):
    ...
    for server_type_index in range(0, len(server_type_category)):
        ...
        for cpu_frequency_index in range(0, len(first_image_frequency_x)):
            ...
            print(select_result)
            result.append(float(select_result[0]['iops_read']))
            print()
            print()
